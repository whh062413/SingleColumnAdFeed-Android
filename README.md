...
在 app/src/main/java/com/wuyiming/singlecolumnadfeed_android/config/ 下新建 AiConfig.java：
...
## 📁 项目结构
    ...
    │
    ├── network/
    │   ├── AiApiService.java              # Deepseek API 调用
...
本项目在以下方面的开发过程中使用了Codex辅助 ：

- **Compose UI 层**：AdCardFactory.kt、ScreenStateView.kt、FeedScreen.kt、DetailScreen.kt、Theme.kt 等 Jetpack Compose 组件的初始生成与迭代优化，包括大图/小图/视频三种卡片布局、骨架屏 shimmer 动画
- **AI 配置模块**：AiConfig.java、AiInsightGenerator.java、AiSearchService.java、AiApiService.java 等 AI 服务接入层的设计与实现
- **流程设计**：下拉刷新分页加载流程、AI 云端 → 本地降级策略、视频自动播放与缓存策略
- **模块设计**：MVVM + Repository 架构选型、数据层全部 Java + UI 层保留 Kotlin 的混合语言策略、视频播放器池复用模型、卡片工厂模式（AdCardFactory）
- **埋点追踪**：AdTracker.java 曝光/点击埋点、TrackingModels.java 事件模型的设计与实现
